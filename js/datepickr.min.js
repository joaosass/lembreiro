...
a.element.nodeName?"focus":"click"};H=function(){a.addEventListener(a.element,y(),z);a.addEventListener(l,"click",F)};z=function(){a.addEventListener(document,"click",s);};A=function(){a.removeEventListener(document,"click",s);hideT()};I=function(){var b,c;a.removeEventListener(document,"click",s);a.removeEventListener(a.element,y(),z);b=a.element.parentNode;b.removeChild(l);c=b.removeChild(a.element);b.parentNode.replaceChild(c,b)};(function(){var b,e;a.config={};a.destroy=I;for(b in k)a.config[b]=c[b]||k[b];a.element=d;a.element.value&&(e=Date.parse(a.element.value));e&&!isNaN(e)?(e=new Date(e),a.selectedDate={day:e.getDate(),month:e.getMonth(),year:e.getFullYear()},a.currentYearView=a.selectedDate.year,a.currentMonthView=a.selectedDate.month,a.currentDayView=a.selectedDate.day):(a.selectedDate=null,a.currentYearView=f.year(),a.currentMonthView=f.month.integer(),a.currentDayView=f.day());B();G();H()})();return a};
datepickr.init.prototype={hasClass:function(d,c){return d.classList.contains(c)},addClass:function(d,c){d.classList.add(c)},removeClass:function(d,c){d.classList.remove(c)},forEach:function(d,c){[].forEach.call(d,c)},querySelectorAll:document.querySelectorAll.bind(document),isArray:Array.isArray,addEventListener:function(d,c,f,h){d.addEventListener(c,f,h)},removeEventListener:function(d,c,f,h){d.removeEventListener(c,f,h)},l10n:{weekdays:{shorthand:"Dom Seg Ter Qua Quin Sex Sab".split(" "),longhand:"Sunday Monday Tuesday Wednesday Thursday Friday Saturday".split(" ")},
months:{shorthand:"Jan Fev Mar Abr Mai Jun Jul Ago Set Out Nov Dez".split(" "),longhand:"Janeiro Fevereiro Março Abril Maio Junho Julho Agosto Setembro Outubro Novembro Dezembro".split(" ")},daysInMonth:[31,28,31,30,31,30,31,31,30,31,30,31],firstDayOfWeek:0}};

function hideT(){
    document.querySelector(".datepickr-calendar").style.opacity = 0;
    setTimeout(function(){
        document.querySelector(".datepickr-wrapper").setAttribute("class", "datepickr-wrapper");
    },550);
}

function showT(){
    document.querySelector(".datepickr-wrapper").setAttribute("class", "datepickr-wrapper open");
    setTimeout(function(){
        document.querySelector(".datepickr-calendar").style.opacity = 1;
    },50);
}

document.querySelector(".date").onmouseup = function(){
    showT();
}

document.onkeypress = function(e){
    if(document.querySelector(".datepickr-wrapper").classList.contains("open") && (e.which == 27 || e.keyCode == 27)){
        hideT();
    }
    else if(search.style.display == "flex" && (e.which == 27 || e.keyCode == 27)){
        hide();
    }
}
